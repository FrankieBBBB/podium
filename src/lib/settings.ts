...
export const FIELDS: FieldSpec[] = [
  {
    ...
    help: 'Use in-cluster service DNS where possible; an Ingress sends traffic out of the cluster and back.',
    section: 'dispatcharr',
  },
  {
  ...
  },
  {
    ...
    help: 'Ceiling on how many matched streams a channel carries because of the setting above. Counts what is already there, and lowering it never unassigns anything. 0 removes the cap.',
    section: 'behaviour',
    min: 0,
    max: 100,
  },
  ...
  },
  {
    ...
    help: 'Every channel should be checked within this window. A target the pacer sizes each pass against, not a schedule. 1440 is a day.',
    section: 'behaviour',
    scale: 60_000,
    min: 5,
    max: 43_200,
  },
  {
    ...
    help: 'How often Podium considers a pass. Each pass takes only the slice needed to stay inside the freshness target, so a short interval means a steady trickle rather than a full run.',
    section: 'behaviour',
    scale: 60_000,
    min: 1,
    max: 1_440,
  },
  {
    ...
    help: 'When nothing is due, Podium sleeps until the next verdict expires. This caps that sleep, so a stream the provider added is still noticed within it.',
    section: 'behaviour',
    scale: 60_000,
    min: 1,
    max: 1_440,
  },
  {
    ...
    help: 'How long the EPG grid is reused before re-fetching it. Each pass re-derives what is airing now from the cached rows, so an hour or two is safe unless your grid lists only a short window ahead. 0 disables caching.',
    section: 'behaviour',
    scale: 60_000,
    min: 0,
    max: 1_440,
  },
  {
    ...
    help: 'How long a working stream is trusted before it is checked again. Most of the cache is this, so it is the main lever on how much probing happens.',
    section: 'behaviour',
    scale: 60_000,
    min: 5,
    max: 43_200,
  },
  {
    ...
    help: 'How soon a stream that just died is rechecked. Shorter than the live lifetime because a stream that has only just failed is the most likely to come back.',
    section: 'behaviour',
    scale: 60_000,
    min: 5,
    max: 43_200,
  },
  {
    ...
    help: 'Each consecutive dead verdict doubles the lifetime above, up to this. Stops a permanently dead stream being re-probed forever. Set it equal to the dead lifetime to turn the backoff off.',
    section: 'behaviour',
    scale: 60_000,
    min: 5,
    max: 43_200,
  ...
  },
  {
    ...
    help: 'Off, the Sync button on the Quality page is the only thing that writes to Teamarr. On, the same push also runs on the interval below. Either way a push is refused unless it can be shown not to make the ordering worse.',
    section: 'teamarr',
  },
  {
    ...
    help: 'Only used when the schedule above is on. A rule set moves by single points from one day to the next, so pushing more often achieves little. 24 is daily.',
    section: 'teamarr',
    scale: 3_600_000,
    min: 1,
    max: 720,
  },
  {
    ...
    help: 'A push is refused until the profile is fitted on at least this many in-scope samples. Guards the cleared-database case, where a few hours of samples would otherwise produce confident-looking rules.',
    section: 'teamarr',
    min: 0,
    max: 1_000_000,
  },
  ...
  },
  {
    ...
    help: 'Globs, comma-separated — e.g. “*VOD*, *MOVIE*, *24/7*”. Overrides both settings above: a sample matching either name is dropped however it was admitted. Nothing is deleted, so widening the rules later brings the samples back.',
    section: 'quality',
  },
  {
    ...
    help: 'Across every provider, not per provider. The lane limits protect the providers; this protects the machine, since each probe in flight is an ffprobe and an ffmpeg decoding video. 0 removes the cap.',
    section: 'probing',
    min: 0,
    max: 64,
  },
  ...
  },
  {
    ...
    help: 'How long ffprobe reads before deciding. The biggest single lever on run time: 6s is enough to judge a stream, and below about 3s healthy streams start reading as dead.',
    section: 'probing',
    min: 1,
    max: 60,
  },
